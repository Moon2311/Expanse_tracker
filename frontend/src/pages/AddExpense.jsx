...
export default function AddExpense() {
  ...
  async function submit(e) {
    ...
    setErr("");
    try {
      await api.post("/expenses/", { amount, spent_on: spentOn, description, category });
      nav("/expenses");
    } catch (ex) {
  ...
  return (
    <div className="max-w-md">
      <p className="text-sm mb-4">
        <Link to="/expenses" className="text-accent no-underline">
          ← Expenses
        </Link>
      </p>
      <h1 className="font-display text-2xl text-ink mb-6">Add expense</h1>
